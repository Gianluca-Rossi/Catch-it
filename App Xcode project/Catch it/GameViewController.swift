...
public class GameViewController: UIViewController {
    ...
    var nodesPositions = [String : SCNVector3]()
...
//        } else { fatalError("Unable to load sticks scene file.") }
        ...
        for node in gameScene.rootNode.childNodes {
            // Removes lag when a node enters the viewing frustum for the first time
            forceNodeRendering(node: node)
            if node.name == "bear" || node.name == "panda" || node.name == "spike" || node.name == "penguin" {
    ...
    // Removes lag when a node enters the viewing frustum for the first time
    // In order for this to work, every node in the scene must have a different name
    func forceNodeRendering(node : SCNNode) {
        if let nodeName = node.name {
                nodesPositions[nodeName] = node.position
            node.position = SCNVector3(x: 0, y: -2.6, z: -4.8)
        }
    }
...
//        levelValueText.font = UIFont.systemFont(ofSize: 24, weight: .black).setItalicFnc()
    ...
    func setupNodes() {
        spikeNode = gameScene.rootNode.childNode(withName: "spike", recursively: true)!
        pandaNode = gameScene.rootNode.childNode(withName: "panda", recursively: true)!
        bearNode = gameScene.rootNode.childNode(withName: "bear", recursively: true)!
        penguinNode = gameScene.rootNode.childNode(withName: "penguin", recursively: true)!
        ...
        for node in gameScene.rootNode.childNodes {
            if let nodeName = node.name {
                if let defaultNodePosition = nodesPositions[nodeName] {
